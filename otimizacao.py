...
# Plotagem do caminho percorrido pelo método de gradiente descendente em 2D
plt.figure()
plt.plot(pontos_x_gradiente, pontos_y_gradiente, marker='o')
for i, (x, y) in enumerate(zip(pontos_x_gradiente, pontos_y_gradiente)):
...
# Plotagem do caminho percorrido pelo método de Newton em 2D
plt.figure()
plt.plot(pontos_x_newton, pontos_y_newton, marker='o')
for i, (x, y) in enumerate(zip(pontos_x_newton, pontos_y_newton)):
    plt.text(x, y, str(i+1), fontsize=12, color='blue', ha='center', va='center')
plt.title('Caminho de Newton')
plt.xlabel('X')
plt.ylabel('Y')
plt.grid(True)
plt.show()